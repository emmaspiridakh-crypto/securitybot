...
class AntiNuke(commands.Cog):
    ...
    async def on_member_ban(self, guild: discord.Guild, user: discord.User):
        ...
        if moderator.bot or await Database.is_server_owner(str(moderator.id), self.bot.installer_id):
            return
        limit  = int(await Database.get_config("mass_action_limit",  "3"))
        window = int(await Database.get_config("mass_action_window", "5"))
    ...
    async def on_guild_channel_delete(self, channel: discord.abc.GuildChannel):
        ...
        if moderator.bot or await Database.is_server_owner(str(moderator.id), self.bot.installer_id):
            return
        limit  = int(await Database.get_config("mass_action_limit",  "3"))
        window = int(await Database.get_config("mass_action_window", "7"))
        key    = f"ch_del_{guild.id}_{moderator.id}"
        if channel_del_tracker.add_and_check(key, limit, window):
            channel_del_tracker.reset(key)
            await Database.log_event("channel_delete", {"user": str(moderator), "channel": channel.name})
            member = guild.get_member(moderator.id)
            if member:
